...
def single_updates(stops):
    for s in stops:
        payload = { 'lines': s['lines'] }
        headers = { 'X-Parse-Application-Id': args.appid,
                    'X-Parse-REST-API-Key': args.key,
                    'Content-Type': 'application/json' }
        url = PARSE_API + '/1/classes/' + args.classname + '/' + s['objectId']
        print(url, payload)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--linedatafile', help='Filename of JSON file containing new data', default='linedata.json')
    parser.add_argument('-c', '--classname', help='Name of Parse object class', default='Stop')
    parser.add_argument('-d', '--dirdatafile', help='Name of file with stop codes and running directions', default='stop_directions.csv')
    ...
    stops = []
    with open(args.linedatafile) as json_file:
        stops = json.load(json_file)    

    dirs = read_dirs(args.dirdatafile)
    print('dirs = ', len(dirs))
    print(type(dirs))
